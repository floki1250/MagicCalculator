...
<template>
    <UPopover mode="hover" :ui="{ width: 'w-[156px]' }">
        <template #default="{ open }">
            <UButton color="gray" variant="ghost" square :class="[open && 'bg-gray-50 dark:bg-gray-800']"
                aria-label="Stroke picker" icon="i-ph-line-segment">
                {{ current }}
            </UButton>
        </template>
            ...
            <div class="p-2">
                <div class="flex flex-col gap-2">
                    <div v-for="size in sizes" :key="size"
                        ...
                        :style="{ backgroundColor: color, height: `${size * ratio + 4}px` }" @click="setSize(size)" />
                </div>
            </div>
        </template>
    </UPopover>
